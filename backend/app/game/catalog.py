...
CATALOG = [
    ...
    },
    {
        ...
        "witch": "一次复活当夜由自己造成死亡的角色（不要求目标曾受庇护），该角色成为不能投票、不能发动技能的傀儡，那次死亡的公告与死因一并撤销；被魔女刀、额外攻击或全场攻击击中而留下的目击不受复活影响；可隐藏自己使用13水杀人的死因。",
    },
    {
        "id": "noah",
        "name": "诺亚",
    ...
    },
    {
        ...
        "normal": "6颗子弹，仅在即将被处决时开枪：每开一枪前重新选择目标，可以一直连发到子弹用完，也可以主动收手；连续未命中会把下次命中率由1/6逐步提高至必定命中，命中后重置。白天可幻视本日处决名单是否含魔女。",
        "witch": "每夜获知全员当前魔女化状态。",
    },
    {
        "id": "arisa",
...
# 技能播报里用的短名：动作标签是一句完整的话（「宣布爱上或移情」），
# 播报卡片要的是技能名本身，所以单独一张表，避免改动动作标签影响别处文案。
ABILITY_NAMES = {
    "interrupt": "打断发言",
    "mass_brainwash": "全场洗脑",
    "love": "爱上/移情",
...
# 技能的公开介绍：全部逐句取自 CATALOG 里已经公开的角色说明，不新增规则文本。
# 播报卡片点击后展示它，与点头像看到的角色详情同源。
ABILITY_INTRO = {
    "interrupt": "即使在下层，每个白天也可打断一次他人发言。",
    "mass_brainwash": "一次洗脑全场处决一人；次日失去投票权且必须被处刑。",
    "love": (
        "白天可宣布爱上一人或移情（从当天夜里开始生效）；若爱人整席出局则转爱自己。"
        "每夜令爱人席当前牌负伤一次，同时爱人免疫处决、临刑开枪、魔女刀与13水等一切死亡和其他负伤。"
    ),
    "gaze": "白天可幻视本日处决名单是否含魔女。",
    "duel": (
        "白天可宣布与一张当前牌决斗并失去本技能：今天所有人必须至少同意这两张牌之一，"
        "且它们达到半数即可处决。"
    ),
    "photo": "白天可赠送无图像信物；受赠者可以授权你观看自己的夜间行动。",
}

# 声明当场就把目标公开的技能：打断的对象就是当前发言者、决斗对手与被打入处决名单者
# 都已经由效果本身公开。爱上/移情的对象是私密情报，赠送信物的受赠者也只私下收到通知。
PUBLIC_TARGET_ABILITIES = {"interrupt", "duel", "mass_brainwash"}
