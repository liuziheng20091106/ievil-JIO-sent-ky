...
- **发新版必须写更新日志**（`data/updates.json` 的 title/notes，两个平台各自一份）：发布时自动从 git 提交里找**客户端改动**——上一次发版（上一个把 `client/lib/src/client_version.dart` 或 `client/pubspec.yaml` 版本号改上去的提交，见 `git log -- client/lib/src/client_version.dart client/pubspec.yaml`）以来的全部提交，逐个看涉及 `client/`（以及 Windows 更新器）的 diff 归纳成用户能读懂的条目，按「新功能 → 修复 → 其它」分组写成 markdown；纯后端/文档/发布脚本的改动不进客户端日志。写完把 title 里的版本号同步改成新版本，Windows 与 Android 的 notes 可以按平台差异各写各的。日志写错版本或照抄上一版是事故，不是小事。
