...
- 主持授权只说明「有资格主持」：主持账号进入某一局还要先确认（`POST /api/games/{id}/host/enter`，记在 `game["host_entries"]`），未确认前只有观察者投影——没有全席上下牌、主持人面板、私聊历史，也执行不了任何命令（含禁言、移人、替补、代操作）。放行判据只有 `game.state.host_capable(actor)` 一处，投影、消息可见性、证物、成就与命令校验都走它；非本局主持人确认进入时另发一条全服通告。
