...
- `package-release.cmd` 已是纯 ASCII，中文路径与码页不再影响它，DSH/自动化里直接 `cmd /c package-release.cmd` 即可（不再需要 `chcp 936`，`.venv` 的 python 用 `-X utf8` 运行，输出的中文按 UTF-8 管道可正常解码）。`--dry-run` 只打包并打印计划（不联网、不改 `downloads.json`），`--skip-zip` 复用已有压缩包，`--no-downloads` 跳过写下载链接。打包前后会比对发行目录的文件指纹，`flutter build` 还在跑时拒绝出包，而不是打进去一个新旧混合的半成品（退出码 1 并提示等构建结束）；`seven_double_client.exe` 不存在也按同一情况处理。
...
- 应用内更新与用户协议：客户端所有请求带 `seven-double-flutter/<版本> (<平台>)` 的 UA；`/api/online` 只回一句「有没有更新」，有更新时客户端再请求 `/api/health` 取更新详情。`data/agreement.md` 是首次连接服务器时展示的用户协议（Markdown），`data/releases/` 是可选的自托管更新包目录（`GET /releases/{文件名}`）。发布客户端时除了 `package-release.cmd` 还要跑 `.venv\Scripts\python.exe tools\publish-client-update.py`：它上传 `Updater.exe` 并刷新 `data/updates.json` 里两条兜底区间的 latest/url/size/sha256（手工写的 title/notes/minimum/guide_url 与更窄区间条目保留）。
