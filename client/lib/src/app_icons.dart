...
/// 行动分区的主色，用于图标底色。
Color actionTint(String group, {bool danger = false}) {
  if (danger) return AppColors.danger;
  return switch (group) {
    '开局' || '准备' => AppColors.host,
    '私密管理' || '私密信息' => AppColors.host,
    '投票' => AppColors.accent,
    '热气球' => AppColors.info,
    '房间管理' => AppColors.textSecondary,
    '私信' => AppColors.info,
    '流程' => AppColors.accent,
    _ => AppColors.accent,
  };
...
class ActionIconBadge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final tint = actionTint(group, danger: danger);
    return Container(
      width: size,
      height: size,
        ...
        child: Image.asset(
          ...
          errorBuilder: (context, error, stack) => Container(
            ...
            child: Icon(Icons.local_florist_outlined,
                size: size * .5, color: AppColors.accent),
          ),
        ),
      );
}
