...
try {
  ...
  tabId = navigation?.type === "reload" || navigation?.type === "back_forward"
    ? sessionStorage.getItem(`${prefix}tab`) ?? ""
    : "";
  tabId ||= Array.from(crypto.getRandomValues(new Uint32Array(4)), (part) => part.toString(16)).join("-");
...
export function draftKey(gameId: string | null, actorId: string | null, ...parts: unknown[]) {
...
export function useDraft<T>(key: string, initial: T | (() => T)):
  ...
  if (!draft) {
    ...
    try {
      ...
      if (stored !== null) {
        ...
        if (!parsed || typeof parsed !== "object" || !("value" in parsed)) throw new Error("草稿损坏");
        const saved = parsed.value;
        if ((saved === null) !== (fallback === null) ||
          typeof saved !== typeof fallback || Array.isArray(saved) !== Array.isArray(fallback)) throw new Error("草稿格式不符");
        value = saved as T;
      }
    } catch {
      error = "本地草稿无法恢复或存储不可用；当前填写仅保留在本页，请勿刷新或关闭。";
    }
    draft = { snapshot: { value, error, revision: 0 }, initial: fallback, listeners: new Set(), active: true };
  ...
  const clear = () => {
    ...
    entry.snapshot = { value: error ? snapshot.value : entry.initial, error, revision: snapshot.revision + 1 };
    notify(entry);
...
export function clearActorDrafts(actorId: string): string {
  ...
  for (const [key, draft] of drafts) {
    ...
    draft.active = false;
    draft.snapshot = { value: draft.initial, error: "", revision: draft.snapshot.revision + 1 };
    notify(draft);
    drafts.delete(key);
  }
  try {
    ...
    for (let index = 0; index < localStorage.length; index++) {
      ...
      if (key?.startsWith(namespace) && belongsToActor(key.slice(namespace.length))) keys.push(key);
    }
