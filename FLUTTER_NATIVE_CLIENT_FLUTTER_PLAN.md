...
5. Flutter 执行 `flutter analyze`、最小 DTO/地址解析检查、`flutter build apk --debug`、`flutter build windows --debug`。Android 真机验证 QQ 登录持久恢复、底栏角标、触觉、软键盘隐藏行动列表但保留新行动提醒、阶段动画只出现一次、双头像 50% 重叠/当前牌置顶/死亡灰白；Windows 验证独立主持人工作台、无触觉，以及禁止重复实例：第一实例运行中、刚启动未就绪、最小化三种情况下重复启动都只唤前台并立刻退出，进程数保持 1，全部退出后能再次启动。
